...
from sklearn.metrics import (
    ...
    precision_recall_curve, accuracy_score, roc_curve
)
...
EXPORT_NAME_PREFIX    = "model7round2"
...
# ====== Plots ======
def plot_cm_only(y_true, pred, export_dir, ts, prefix="report"):
    ...
    plt.colorbar()
    ticks = np.arange(2)
    plt.xticks(ticks, ["benign", "malicious"], rotation=45, ha="right")
    plt.yticks(ticks, ["benign", "malicious"])
    th = cm.max() / 2.0
...
def plot_family_split_stacked(mmap_info, family_arr, splits, export_dir, ts, prefix="report"):
    ensure_dir(export_dir)
    ...
    def cnt(arr):
        ...
        for fid in np.unique(arr):
            d[inv[int(fid)]] = int((arr == fid).sum())
        return d

    fam_tr, fam_va, fam_te = cnt(family_arr[tr]), cnt(family_arr[va]), cnt(family_arr[te])
    fams = sorted(set(list(fam_tr.keys()) + list(fam_va.keys()) + list(fam_te.keys())))
    ...
    denom = np.maximum(trc + vac + tec, 1.0)
    trp, vap, tep = 100.0 * trc / denom, 100.0 * vac / denom, 100.0 * tec / denom

    plt.figure(figsize=(10, 4))
    plt.bar(fams, trp, label="TRAIN")
    plt.bar(fams, vap, bottom=trp, label="VAL")
    plt.bar(fams, tep, bottom=trp + vap, label="TEST")
    plt.title("Split Composition per Family")
    plt.ylabel("%"); plt.legend(); plt.xticks(rotation=25, ha="right")
...
def main():
    ...
    ts = ts_now(); times = {}
    def t_start(key):
        times[key] = {"start": time.perf_counter(),
                    "wall_start": datetime.datetime.now().isoformat(timespec='seconds')}
    def t_end(key):
        t = time.perf_counter() - times[key]["start"]
        times[key]["sec"] = t
        times[key]["wall_end"] = datetime.datetime.now().isoformat(timespec='seconds')
...
# -- Split (family/group-aware quota)
    ...
    # plots
    t_start("plots")
    plot_family_split_stacked(mmap_info, family, splits, EXPORT_DIR, ts, prefix=EXPORT_NAME_PREFIX)
    plot_cm_only(y_true, pred_test, EXPORT_DIR, ts, prefix=EXPORT_NAME_PREFIX)
