...
"""
Train 1D-CNN on packet payload (fixed length with PAD=-1), robust split/rebalance,
robust threshold, plots + final report. Version: full-fixed

Highlights:
- Robust decode (base64 normal/urlsafe, hex with validation, latin-1 fallback)
- Count zero-length-after-decode per family (logging)
- Memmap all arrays, tf.data pipeline with .repeat() for training
- Family/group-aware split + per-family rebalance (train oversample)
- Model with explicit mask handling (no anonymous lambdas) to avoid deserialization issues
- Mixed precision optional; safe masked GlobalMaxPooling in float32
- Saves BEST/FINAL models, meta, plots, history, final_report
"""
...
RESUME_LR             = 1e-4          # ใช้ตอน continue; None = ใช้ LR เดิม
EVAL_ONLY             = False
BASE_MODEL_PATH       = "/home/intgan/Desktop/jek/train/scratch(8)round1.final.20250928-015630.keras"
...
EXPORT_PREFIX  = "token1460_round7_threshold"
...
b64_chars_re = re.compile(r'^[A-Za-z0-9_\-+/=\s]+$')   # allow urlsafe - _
...
def decode_payload(s: str) -> bytes:
    """Robust decoder: try base64 (normal -> urlsafe), then hex (even length), else latin-1."""
    if not isinstance(s, str): return b""
    s = s.strip()
    if not s: return b""

    # base64 (normal / urlsafe)
    if b64_chars_re.match(s) and len(s.replace("\n","").replace(" ","")) >= 4:
        try:
    ...
    # hex (must be even length after removing whitespace)
    if hex_re.match(s):
        hs = re.sub(r"\s+","", s)
...
def to_fixed_len_int32(b: bytes, L=FIXED_LEN) -> np.ndarray:
    """Return int32 vector of length L: real bytes 0..255; PAD = -1 (no truncation except >L)."""
    arr_u8 = np.frombuffer(b, dtype=np.uint8)
    n = len(arr_u8)
    if n > L:
        print(f"[WARN] payload length {n} > L={L} (truncate). Consider raising FIXED_LEN.")
        arr_u8 = arr_u8[:L]; n = L
    out = np.full(L, -1, dtype=np.int32)
    if n > 0:
        out[:n] = arr_u8.astype(np.int32)
    return out

# =================== History/Plots helpers ===================
...
#     print("[MMAP] counting ...")
#     total = 0; raw_by = {}; eff_by = {}; max_len_seen = 0
#     for p, fam in norm:
...
#     fam2id, grp2id = {}, {}; w = 0
#     zero_by = {}  # count zero-length-after-decode per family
...
#             vecs, keep = [], []
#             for s_val in chunk[PAYLOAD_COL].values:
#                 b = decode_payload(s_val)
#                 if len(b) == 0:
#                     keep.append(False)
#                     zero_by[fam] = zero_by.get(fam, 0) + 1
#                 else:
#                     vec = to_fixed_len_int32(b, L=L)
#                     vecs.append(vec); keep.append(True)
#                     if len(b) > max_len_seen: max_len_seen = len(b)
...
#     print(f"[MMAP] done: {w}/{total} (N_eff/N_total)")
#     print(f"[MMAP] max payload length seen = {max_len_seen}, FIXED_LEN = {L}")
#     if zero_by:
#         print("[MMAP] zero-length per family:", {k:int(v) for k,v in zero_by.items()})
#         print("[MMAP] zero-length total =", int(sum(zero_by.values())))
...
def build_memmap_from_csvs(csv_files, L=FIXED_LEN, mmap_dir=MMAP_DIR):
    """
    อ่าน CSV หลายไฟล์ -> decode payload -> แปลงเป็น int32 vector ขนาดคงที่ L
    กรอง payload ที่ว่างหรือสั้นเกินไป (<16 bytes) ออกโดยอัตโนมัติ
    เขียนข้อมูลทั้งหมดลง memmap เพื่อลดการใช้ RAM
    """
    ensure_dir(mmap_dir)
    norm = _normalize_csvs(csv_files)
    print("[MMAP] counting ...")
    total = 0; raw_by = {}; eff_by = {}; zero_by = {}; short_by = {}; max_len_seen = 0

    # นับจำนวนแถวต่อ family
    for p, fam in norm:
        n = _count_rows_quick(p)
        raw_by[fam] = raw_by.get(fam, 0) + n
        total += n
        print(f"  - {Path(p).name}: {n} (family={fam})")
    ...
    for p, fam in norm:
        ...
        for chunk in pd.read_csv(p, usecols=[PAYLOAD_COL, LABEL_COL],
            ...
            for s_val in chunk[PAYLOAD_COL].values:
                b = decode_payload(s_val)
                # ⚙️ กรอง payload ว่างหรือสั้นกว่า 16 bytes ออก
                if len(b) == 0:
                    zero_by[fam] = zero_by.get(fam, 0) + 1
                    continue
                if len(b) < 16:
                    short_by[fam] = short_by.get(fam, 0) + 1
                    continue

                # เขียนข้อมูลที่ผ่านการกรอง
                vec = to_fixed_len_int32(b, L=L)
                vecs.append(vec)
                if len(b) > max_len_seen:
                    max_len_seen = len(b)
            ...
            n = len(arr)
            X_all[w:w+n] = arr
            y_all[w:w+n] = chunk[LABEL_COL].values[:n].astype(np.float32)
            groups[w:w+n] = grp_id
            family[w:w+n] = fam_id
            w += n
            eff_by[fam] = eff_by.get(fam, 0) + n
    ...
    if zero_by:
        ...
        print("[MMAP] zero-length total =", int(sum(zero_by.values())))
    if short_by:
        print("[MMAP] short(<16B) per family:", {k: int(v) for k, v in short_by.items()})
        print("[MMAP] short total =", int(sum(short_by.values())))

    return {
        "X_path": X_path, "y_path": y_path, "g_path": g_path, "f_path": f_path,
        "N_total": total, "N_eff": w, "L": L,
        "fam2id": fam2id, "grp2id": grp2id,
        "raw_rows_by_fam": raw_by, "eff_rows_by_fam": eff_by,
        "zero_by": zero_by, "short_by": short_by
    }
...
def split_family_group_aware_quota(groups, family, y,
    ...
    for f in fams:
        ...
        if tot > n_f:
            ...
            while (n_train + n_val + n_test) > n_f: n_train -= 1

        # NOTE: disable strict group split; use random within-family indices (stable)
        ...
        # ensure each of val/test has both labels (if possible)
...
def rebalance_split_to_targets(idx_split, family_arr, fam2id, benign_name="benign",
    ...
    if targets is not None:
        ...
        if benign_id is None:
            ...
            for fid in list(quota.keys())[:rem]: quota[fid] += 1
        else:
...
def make_dataset_from_indices(X, y, indices, batch, training=False, seed=SEED):
    ...
    ds = tf.data.Dataset.from_generator(
        gen,
        output_signature=(
            tf.TensorSpec(shape=(FIXED_LEN,), dtype=tf.int32),
...
# =================== Model (no anonymous lambdas) ===================
# Small helper layers to avoid Lambda(lambda ...) serialization issues
class ByteIdLayer(layers.Layer):
    """Map {-1,0..255} -> {0..256} where 0 is PAD id."""
    def call(self, t):
        z = tf.zeros_like(t)
        return tf.where(t < 0, z, t + 1)  # shape unchanged

class PadMaskLayer(layers.Layer):
    """Return mask 1.0 for real bytes (>=0), 0.0 for PAD (-1)."""
    def call(self, t):
        return tf.cast(t >= 0, tf.float32)  # (B, L)

class ExpandLastDim(layers.Layer):
    def call(self, m):
        return tf.expand_dims(m, -1)  # (B, L, 1)

class CastF32(layers.Layer):
    def call(self, x):
        return tf.cast(x, tf.float32)

class ApplyVeryNegMask(layers.Layer):
    """Subtract big number at PAD positions to make GlobalMaxPooling ignore PAD."""
    def call(self, inputs):
        x32, m32 = inputs
        very_neg = (1.0 - m32) * 1e4  # PAD -> 1e4, data -> 0
        return x32 - very_neg

def make_model(L=FIXED_LEN, emb_dim=16, filters=128, k=7, drop=0.25):
    inp = layers.Input(shape=(L,), dtype=tf.int32)  # {-1}∪[0..255]
    ids  = ByteIdLayer(name="byte_ids")(inp)        # 0..256 (0=PAD)
    mask = PadMaskLayer(name="pad_mask")(inp)       # (B, L)
    mask_exp = ExpandLastDim(name="mask_expand")(mask)  # (B, L, 1)

    x = layers.Embedding(input_dim=257, output_dim=emb_dim, mask_zero=False, name="byte_embed")(ids)

    def block(x, dilation, name):
        x = layers.Conv1D(filters, k, padding="same", dilation_rate=dilation,
                          kernel_regularizer=l2(1e-5), name=f"{name}_conv")(x)
        x = layers.BatchNormalization(name=f"{name}_bn")(x)
        x = layers.Activation("relu", name=f"{name}_relu")(x)
        return layers.Dropout(drop, name=f"{name}_drop")(x)

    x = block(x, 1, "b1")
    x = block(x, 2, "b2")
    x = block(x, 4, "b3")

    x32 = CastF32(name="cast_f32")(x)
    m32 = CastF32(name="mask_f32")(mask_exp)
    x_masked = ApplyVeryNegMask(name="apply_veryneg")([x32, m32])

    x = layers.GlobalMaxPooling1D(name="gmp")(x_masked)  # -> float32
    x = layers.Dense(256, activation="relu", kernel_regularizer=l2(1e-5), name="head_dense")(x)
    x = layers.Dropout(0.35, name="head_drop")(x)
...
def choose_threshold(y_true, proba, policy="best_f1", target_recall=0.90,
                     min_precision=None, min_thr=0.0):
    proba = np.asarray(proba)
    pmin, pmax = float(np.min(proba)), float(np.max(proba))
    prec, rec, thr = precision_recall_curve(y_true, proba)

    def _clamp(t):
        # อย่าให้ threshold อยู่นอกช่วง proba จริง
        eps = max(1e-12, 0.001*(pmax-pmin+1e-12))
        return float(np.clip(t, pmin+eps, pmax-eps))

    if policy == "best_f1":
        f1 = 2*prec[1:]*rec[1:] / (prec[1:]+rec[1:]+1e-9)
        i  = int(np.argmax(f1))
        t  = _clamp(thr[i] if i < len(thr) else 0.5)
        if min_precision is not None and prec[i+1] < min_precision:
            j = i + 1
            while j < len(prec) and prec[j] < min_precision:
                if j-1 < len(thr): t = _clamp(thr[j-1])
                j += 1
        return t, {"policy":"best_f1","F1":float(f1[i]),
                   "Precision":float(prec[i+1]),"Recall":float(rec[i+1]),
                   "proba_range":[pmin,pmax]}

    if policy == "target_recall":
        idx = np.where(rec >= target_recall)[0]
        if len(idx) > 0:
            i = int(idx[0])
            t = _clamp(thr[max(i-1,0)] if i > 0 else (pmax+pmin)/2.0)
            if min_precision is not None:
                j = i
                while j < len(prec) and prec[j] < min_precision:
                    if j-1 < len(thr): t = _clamp(thr[j-1])
            ...
            return t, {"policy":"target_recall",
                       ...
                       "Recall_at_chosen":float(rec[min(i,len(rec)-1)]),
                       "proba_range":[pmin,pmax]}
    # fallback: median ของ proba
    t = float(np.median(proba))
    return t, {"policy":"median_fallback","proba_range":[pmin,pmax]}
...
def main():
    ...
    if REUSE_SPLIT_FROM_META and os.path.exists(PREV_META_PATH):
        ...
        if max_idx >= N_eff:
            raise RuntimeError(f"[SPLIT] Reused indices out of range (max={max_idx}, N_eff={N_eff}).")
    ...
    # quick pad ratio peek
    def debug_print_pad_ratio(X_all, indices, k=3, name="SET"):
        if len(indices) == 0: print(f"[DEBUG][{name}] empty"); return
        rng = np.random.RandomState(SEED)
        take = indices if len(indices) <= k else rng.choice(indices, size=k, replace=False)
        for idx in take:
            x = X_all[int(idx)]
            pad_ratio = float((x == -1).mean())
            print(f"[DEBUG][{name}] sample={int(idx)} pad_ratio={pad_ratio:.4f} first20={x[:20].tolist()}")
    debug_print_pad_ratio(X_all, train_idx, 3, "TRAIN")
    debug_print_pad_ratio(X_all, val_idx,   2, "VAL")
    debug_print_pad_ratio(X_all, test_idx,  2, "TEST")

    # Overfit mini-debug (optional)
    if OVERFIT_DEBUG:
        rng = np.random.RandomState(SEED)
        if len(train_idx) > OVERFIT_N:
            train_idx = np.sort(rng.choice(train_idx, size=OVERFIT_N, replace=False))
            print(f"[DEBUG][OVERFIT] use {OVERFIT_N} samples")
            print_dataset_stats("TRAIN(OVERFIT)", y_all[train_idx], family[train_idx], fam2id)

    # Optional cap per family on TRAIN
    if MAX_PER_FAMILY_TRAIN is not None:
        rng=np.random.RandomState(SEED); keep=[]
        fams = np.unique(family[train_idx])
        for f in fams:
            rel = np.where(family[train_idx]==f)[0]
            take = rng.choice(rel, size=min(len(rel), MAX_PER_FAMILY_TRAIN), replace=False)
            keep.append(take)
        keep_rel=np.concatenate(keep) if keep else np.arange(len(train_idx))
        train_idx=train_idx[keep_rel]
        print_dataset_stats("TRAIN (after cap)", y_all[train_idx], family[train_idx], fam2id)
    ...
    if TRAIN_MODE == "continue" or EVAL_ONLY:
        ...
        model = tf.keras.models.load_model(BASE_MODEL_PATH, custom_objects={
            "ByteIdLayer": ByteIdLayer,
            "PadMaskLayer": PadMaskLayer,
            "ExpandLastDim": ExpandLastDim,
            "CastF32": CastF32,
            "ApplyVeryNegMask": ApplyVeryNegMask
        }, safe_mode=False)
    ...
    print(f"[OPT] Adam LR = {cur_lr:g}")
    model.summary()
    t_end("build_compile")

    # callbacks
    ckpt_path = os.path.join(EXPORT_DIR, f"{EXPORT_PREFIX}.best.{ts}.keras")
    ...
    timer_cb = EpochTimer()
    callbacks = [checkpoint_cb, earlystop_cb, plateau_cb, timer_cb, print_lr]
    ...
    if not EVAL_ONLY:
        ...
        history = model.fit(
            ...
            epochs=EPOCHS,
            steps_per_epoch=steps_per_epoch,   # because train_ds repeats
    ...
    proba_val = np.nan_to_num(proba_val, nan=0.5, posinf=1.0, neginf=0.0)
    print("[DEBUG] VAL proba min/mean/max:",
          float(np.min(proba_val)), float(np.mean(proba_val)), float(np.max(proba_val)))
    hist_val = np.histogram(proba_val, bins=[0,1e-6,1e-3,1e-2,1e-1,0.5,0.9,0.99,1.0])
    print("[DEBUG] VAL proba histogram (counts):", hist_val[0])
    t_end("val_predict")
    ...
    proba_test = np.nan_to_num(proba_test, nan=0.5, posinf=1.0, neginf=0.0)
    print("[DEBUG] TEST proba min/mean/max:",
          float(np.min(proba_test)), float(np.mean(proba_test)), float(np.max(proba_test)))
    print("[DEBUG] TEST %<=1e-6:", float((proba_test <= 1e-6).mean()))
    hist_test = np.histogram(proba_test, bins=[0,1e-6,1e-3,1e-2,1e-1,0.5,0.9,0.99,1.0])
    print("[DEBUG] TEST proba histogram (counts):", hist_test[0])
    t_end("test_predict")
    ...
    # plots
    t_start("plots")
    plot_family_mix_charts(mmap_info, family, splits, EXPORT_DIR, ts, prefix=EXPORT_PREFIX)
    plot_curves_and_cm(y_true, proba_test, pred_test, EXPORT_DIR, ts, prefix=EXPORT_PREFIX)
    ...
    def plot_time_charts_local():
        ...
        out1 = os.path.join(EXPORT_DIR, f"{EXPORT_PREFIX}.{ts}.time_sections.png")
        plt.tight_layout(); plt.savefig(out1, dpi=150); plt.close()

        epoch_times = []
        for cb in [c for c in [timer_cb] if isinstance(c, EpochTimer)]:
        ...
        if len(epoch_times)>0:
            ...
            out2 = os.path.join(EXPORT_DIR, f"{EXPORT_PREFIX}.{ts}.epoch_times.png")
            plt.tight_layout(); plt.savefig(out2, dpi=150); plt.close()
            print("[TIME PLOTS]\n  ->", out1, "\n  ->", out2)
        else:
            print("[TIME PLOTS]\n  ->", out1)
        ...
        if (history is not None) and ("val_PR-AUC" in history.history):
            ...
            out3 = os.path.join(EXPORT_DIR, f"{EXPORT_PREFIX}.{ts}.val_pr_auc_per_epoch.png")
    ...
    t_start("save_meta")
    if not EVAL_ONLY:
        final_model_path = os.path.join(EXPORT_DIR, f"{EXPORT_PREFIX}.final.{ts}.keras")
    ...
    if train_secs and history is not None:
        approx_train_samples = len(train_idx)
    ...
    meta = {
        ...
        "decode": "base64/urlsafe->hex(valid even)->latin1", "window_mode": "no-trunc (pad=-1 to L)",
...
# =================== Inference helper ===================
def predict_payload_string(s, model_path, meta_path=None, fixed_len=FIXED_LEN, threshold=None):
    mdl = tf.keras.models.load_model(model_path, custom_objects={
        "ByteIdLayer": ByteIdLayer,
        "PadMaskLayer": PadMaskLayer,
        "ExpandLastDim": ExpandLastDim,
        "CastF32": CastF32,
    ...
    b = decode_payload(s)
    x = to_fixed_len_int32(b, L=fixed_len).astype(np.int32)
