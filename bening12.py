...
PCAP_FILE = "/home/intgan/Desktop/jek/benign/bening12.pcap"
CSV_FILE  = "/home/intgan/Desktop/jek/benign/bening12.csv"
NORMAL_IP = "10.0.0.46"   # IP ของเครื่องปกติจาก README
MAX_PAYLOAD_LEN = 512

def process_pcap_ipv4_only(pcap_file, output_csv):
    with open(output_csv, mode='w', newline='') as csvfile:
        fieldnames = ["timestamp", "src_ip", "dst_ip",
                      "src_port", "dst_port", "protocol",
                      "packet_length", "payload", "label"]
        ...
        with PcapReader(pcap_file) as pcap_reader:
            for pkt in pcap_reader:
                try:
                    ...
                    # เก็บเฉพาะ traffic ที่เกี่ยวข้องกับ NORMAL_IP
                    if not (src_ip == NORMAL_IP or dst_ip == NORMAL_IP):
                        continue

                    pkt_len = len(pkt)
                    time = pkt.time

                    # ดึง protocol + port + payload
                    sport = dport = None
                    raw_bytes = b""
                    if TCP in pkt:
                        sport = pkt[TCP].sport
