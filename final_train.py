...
MIN_THR_FLOOR         = 1e-10          # กัน threshold = 0

PAD_TOKEN = 0 

# ข้อมูล
CSV_FILES = [
    ("/home/intgan/Desktop/jek/zeus(csv)/zeus25-1.csv", "zeus"),
    ("/home/intgan/Desktop/jek/zeus(csv)/zeus25-2.csv", "zeus"),
    ("/home/intgan/Desktop/jek/zeus(csv)/zeus25-3.csv", "zeus"),
    ("/home/intgan/Desktop/jek/zeus(csv)/zeus25-4.csv", "zeus"),
    ("/home/intgan/Desktop/jek/zeus(csv)/zeus25-5.csv", "zeus"),
...
EXPORT_NAME_PREFIX    = "final6_dataset"
...
def to_fixed_len_uint8(b: bytes, L=FIXED_LEN, mode="middle"):
    arr = np.frombuffer(b, dtype=np.uint8)
    n = len(arr)
    
    # ขยับค่า Byte จาก 0-255 เป็น 1-256 (เพื่อให้เลข 0 เป็นค่า Padding เท่านั้น)
    ...
    if n >= L:
        ...
        if mode == "middle": 
            # คำนวณจุดเริ่มต้นจากตรงกลางเพื่อให้ได้ความยาว L พอดี
            start = max((n - L) // 2, 0)
            return arr_shifted[start : start + L]
        return arr_shifted[:L] # กรณี "head"
    
    # กรณีข้อมูลสั้นกว่า L ให้เติม 0 (Padding) ต่อท้าย
...
def build_memmap_from_csvs(csv_files, L=FIXED_LEN, mmap_dir=MMAP_DIR):
    ...
    for p, fam in norm:
        ...
        # -----------------------------------------------------------------
        # [จุดที่แก้ 1] เช็คชื่อว่าใช่ benign ไหม ถ้าไม่ใช่ให้เป้าหมายเป็น 1 ทันที
        # -----------------------------------------------------------------
        target_label = 0 if fam.lower() == "benign" else 1

        for chunk in pd.read_csv(p, usecols=[PAYLOAD_COL, LABEL_COL],
                                 chunksize=200_000, keep_default_na=False, na_filter=False):
            
            # -----------------------------------------------------------------
            # [จุดที่แก้ 2] บังคับทับค่าในคอลัมน์ Label ไปเลย ไม่สนว่าใน CSV จะเขียนว่าอะไร
            # -----------------------------------------------------------------
            chunk[LABEL_COL] = target_label
            ...
            for s in chunk[PAYLOAD_COL].values:
                ...
                if len(b) == 0: keep.append(False)
                    ...
                    vecs.append(to_fixed_len_uint8(b, L=L, mode="middle"))
                    keep.append(True)
...
def make_dataset_from_indices(X, y, indices, batch, training=False, seed=SEED):
    ...
    def gen():
        # สร้าง array จำลองเพื่อไม่ให้กระทบต้นฉบับ
        current_idx = idx.copy()
        
        # 💡 [จุดเปลี่ยนชีวิต!] ถ้าเป็นการเทรน ให้สลับตำแหน่งดัชนีแบบ 100% (Full Shuffle) ทุกครั้งที่เริ่มรอบใหม่
        if training:
            np.random.shuffle(current_idx)
            
        # ป้อนข้อมูลที่ถูกสับเละแล้วเข้าโมเดล
    ...
    if training:
        # ให้รันซ้ำต่อเนื่อง และใส่ Shuffle ซ้อนอีกชั้นเพื่อความชัวร์
        ds = ds.repeat().shuffle(10000, reshuffle_each_iteration=True)
...
# ====== Model (Configurable Layers) ======
def make_model(L=FIXED_LEN, emb_dim=16, filters=96, drop=0.3, num_layers=2):
    # --- 1. Feature Extraction ---
    inp = tf.keras.Input(shape=(L,), dtype=tf.int32)
    x = tf.keras.layers.Embedding(input_dim=257, output_dim=emb_dim, mask_zero=True)(inp)
    x = tf.keras.layers.SpatialDropout1D(0.25)(x)

    branches = []
    for k_size in [3, 5, 7]:
        # Conv1D ปล่อยไว้ ไม่ต้องใส่ L2 เพื่อให้ดึงฟีเจอร์ได้เต็มที่
        b = tf.keras.layers.Conv1D(filters // 3, k_size, padding="same")(x)
    ...
    # --- 2. Classifier Layers ---
    node_steps = [256, 128, 64, 32, 16, 8]
    if num_layers == 6:
        node_steps = [512, 256, 128, 64, 32, 16]

    current_nodes = node_steps[:num_layers] 
    
    for i, nodes in enumerate(current_nodes):
        # [แก้ไขตรงนี้] ใส่ L2 แบบ "อ่อนมากๆ" (1e-6 แทน 1e-5) กลับเข้าไปเพื่อกันท่องจำ
        x = tf.keras.layers.Dense(nodes, activation="relu", kernel_regularizer=l2(1e-6), name=f"dense_{i+1}")(x)
        
        # เพิ่ม Dropout ขั้นต่ำให้หนาขึ้นนิดนึง (จากเดิม max(0.1, ...) เป็น max(0.2, ...))
        dropout_rate = max(0.2, 0.5 - (i * 0.1)) 
        x = tf.keras.layers.Dropout(dropout_rate)(x)

    # Output Layer
    out = tf.keras.layers.Dense(1, activation="sigmoid", dtype='float32', name="output")(x)
...
def main():
    ...
    # --- เพิ่มโค้ดตรวจสอบด้านล่างนี้ทันที ---
    print("\n" + "="*50)
    print("  [SANITY CHECK] LABEL VALIDATION")
    print("="*50)
    inv_fam = {v: k for k, v in fam2id.items()}
    unique_fids = np.unique(family)

    for fid in unique_fids:
        mask = (family == fid)
        avg_label = np.mean(y_all[mask])
        fam_name = inv_fam[int(fid)]
        
        # ตรวจสอบความถูกต้อง
        status = "OK"
        if fam_name.lower() == "benign":
            if avg_label > 0.1: status = "!!! WARNING: SHOULD BE 0 !!!"
        else:
            if avg_label < 0.9: status = "!!! WARNING: SHOULD BE 1 !!!"
            
        print(f"Family: {fam_name:12s} | Mean Label: {avg_label:.4f} | Status: {status}")
    print("="*50 + "\n")
    ...
    # -----------------------------------------------------------------
    # [จุดที่แก้] ให้คำนวณ Class weights แบบอัตโนมัติตามสัดส่วนจริง
    # -----------------------------------------------------------------
    ...
    print(f"Class weight (Auto-balanced): {class_weight}")
    ...
    if TRAIN_MODE == "continue" or EVAL_ONLY:
        ...
        lr_new = RESUME_LR if RESUME_LR is not None else 1e-3
    else:
        print("[MODE] Train from scratch")
        
        # -------------------------------------------------------------
        # [ยืนยัน] ใช้ 2 ชั้น ตามที่ทดสอบแล้วดีที่สุด
        # -------------------------------------------------------------
        model = make_model(num_layers=6)
        lr_new = 1e-3
    ...
    if not EVAL_ONLY:
        ...
        history = model.fit(
            ...
            class_weight=class_weight, # ใช้ตัวแปรที่คำนวณอัตโนมัติแล้ว
            callbacks=callbacks,               
            verbose=1
        )
    ...
    meta = {
        ...
        "decode": "base64->hex->latin1", "window_mode": "middle",
